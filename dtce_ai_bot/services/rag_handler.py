...
Please try rephrasing your question or contact support if the issue persists."""
    ...
    async def _process_rag_with_full_prompt(self, question: str, retrieved_content: str, documents: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt += "You will receive search results from multiple folders. BEFORE answering, filter the results based on the user's intent:\n\n"
            prompt += "**POLICY PROMPT Filtering** ('safety rules', 'company policies', 'what do I need to follow'):\n"
            prompt += "- ONLY use documents from: Policy/, Health & Safety/, IT Policy/, Employment/, Quality/, Operations/\n"
            prompt += "- IGNORE all project documents, even if they mention safety or procedures\n"
            prompt += "- Purpose: Employee compliance and mandatory rules\n\n"
            prompt += "**PROCEDURES PROMPT Filtering** ('how do I use', 'best practices', 'DTCE approach'):\n"
            prompt += "- ONLY use documents from: H2H/, Procedures/, Technical Procedures/\n"
            prompt += "- IGNORE policy documents and project-specific procedures\n"
            prompt += "- Purpose: Guidance on 'how we do things at DTCE'\n\n"
            prompt += "**STANDARDS PROMPT Filtering** ('NZS requirements', 'building codes', 'code sections'):\n"
            prompt += "- ONLY use documents from: Engineering/, Standards/, NZ Standards/\n"
            prompt += "- IGNORE DTCE-specific procedures and project documents\n"
            prompt += "- Purpose: Official NZ engineering standards and codes\n\n"
            prompt += "**PROJECT PROMPT Filtering** ('past projects', 'show me examples', 'lessons learned'):\n"
            prompt += "- ONLY use documents from: Projects/225/, Projects/224/, Projects/223/, etc.\n"
            prompt += "- IGNORE policies, procedures, and standards documents\n"
            prompt += "- Purpose: DTCE project work, client examples, project history\n\n"
            prompt += "**CLIENT PROMPT Filtering** ('client contact', 'work with [client]', 'client history'):\n"
            prompt += "- ONLY use documents from: Projects/ folders that contain client information\n"
            prompt += "- IGNORE general policies and standards\n"
            prompt += "- Purpose: Client relationship management and contact information\n\n"
            ...
            prompt += "**PROJECT REFERENCE PROMPT (Projects/225/, Projects/224/, etc.):**\n"
