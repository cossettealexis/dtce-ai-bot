...
"""
    ...
    async def _process_rag_with_full_prompt(self, question: str, retrieved_content: str, documents: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            prompt += "5. **ALWAYS for POLICY queries**: Even if primary results are good, ALSO provide 'Alternative that might be helpful' and 'General Engineering Guidance' sections to give users comprehensive context\n"
            ...
            prompt += "2. ONLY include sources section IF the retrieved documents actually contain relevant information\n"
            prompt += "3. If retrieved documents don't answer the question, DO NOT show 'Primary Sources' - just provide your answer based on general knowledge\n"
            prompt += "4. If you do include sources, use EXACTLY this format:\n\n"
            prompt += "**Primary Sources:** (ONLY if documents contain relevant information)\n"
            ...
            prompt += "**Alternative that might be helpful:** (ONLY include if relevant documents exist - don't force this section)\n"
            ...
            prompt += "**General Engineering Guidance:** (MANDATORY for policy/procedure queries - provide specific standards or codes)\n"
            ...
            prompt += "CRITICAL RULES FOR SOURCES:\n"
            ...
            prompt += "3. ONLY include 'Primary Sources' section if documents actually answer the question\n"
            prompt += "4. If documents are irrelevant/unhelpful, provide answer without sources section\n"
            prompt += "5. ALWAYS use Markdown link format: [Document Filename](exact_url)\n"
            prompt += "6. Extract the filename from the document name and use it as the link text\n"
            prompt += "7. If you cannot find a 'Link: https://...' for a document, DO NOT include that document\n"
            prompt += "8. EXAMPLE: If you see 'DOCUMENT: Health & Safety Policy.pdf' and 'Link: https://dtcestorage.blob.core.windows.net/suitefiles/Health%20%26%20Safety%20Policy.pdf', format as: [Health & Safety Policy.pdf](https://dtcestorage.blob.core.windows.net/suitefiles/Health%20%26%20Safety%20Policy.pdf)\n\n"
