...
class RAGHandler:
    ...
    async def process_rag_query(self, question: str, project_filter: Optional[str] = None) -> Dict[str, Any]:
        """
        Process query using the exact flow specified with SEMANTIC SEARCH:
        1. Search user's question in document index using SEMANTIC + OR-based keyword query
        2. Retrieve most relevant documents using AI-powered understanding
        3. GPT reads user intent and constructs natural answer
        4. Use retrieved content if relevant, otherwise general knowledge
        """
        try:
            logger.info("Processing query with semantic search", question=question)
            
            # STEP 1: Search user's question using SEMANTIC SEARCH for better intent understanding
            # First try semantic search for intelligent document retrieval
            ...
            logger.info("Semantic + keyword search results", 
                       semantic_results=len([d for d in documents[:20]]),
                       total_documents=len(documents),
                       sample_filenames=[doc.get('filename', 'Unknown') for doc in documents[:3]])
            
            # STEP 2: Retrieve most relevant documents from index with semantic understanding
            if documents:
                # Format retrieved content from the index with semantic relevance
                index_results = []
                for doc in documents[:10]:  # Top 10 most semantically relevant
            ...
            # STEP 3: GPT analyzes user intent with semantic context and constructs natural answer
            prompt = f"""You will now perform the following steps using SEMANTIC SEARCH results:

The user's question was searched in the document index using SEMANTIC SEARCH for intelligent intent understanding, plus keyword OR queries for comprehensive coverage.
From the index, the most semantically relevant documents were retrieved based on AI-powered understanding of the query meaning.

GPT WILL NOW:
- Read and understand the user's intent using the semantically matched content.
...
📎 Retrieved content from semantic + keyword search:
{retrieved_content}
...
- Focus on practical engineering guidance for New Zealand conditions when applicable."""
            ...
            return {
                ...
                'rag_type': 'semantic_intelligent_search'
            }
...
- Focus on providing actual working templates with access links"""
    ...
    def _get_safe_suitefiles_url(self, blob_url: str, link_type: str = "file") -> str:
        ...
        This ensures we NEVER return Azure blob URLs to users.
        """
        if not blob_url:
    ...
    def _format_sources(self, documents: List[Dict]) -> List[Dict]:
        ...
        for doc in documents[:5]:  # Limit to top 5 sources
            sources.append({
                'filename': doc.get('filename', 'Unknown'),
                'url': self._get_safe_suitefiles_url(doc.get('blob_url', '')),
...
- Team skills and specializations"""
    ...
    def _convert_to_suitefiles_url(self, blob_url: str, link_type: str = "file") -> Optional[str]:
        ...
        Returns:
            SuiteFiles URL or None if conversion fails (NEVER returns original blob URL)
        """
        if not blob_url:
            return None
        ...
        try:
            ...
            if not path_part:
                logger.warning("Could not extract path from blob URL", blob_url=blob_url)
                return None
            ...
            logger.debug("Converted blob URL to SuiteFiles", 
                        blob_url=blob_url, 
                        suite_files_url=suite_files_url,
                        link_type=link_type)
